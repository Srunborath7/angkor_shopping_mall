...
exports.getAttendanceKPIs = async (req, res) => {
  try {
    ...
    let allRecords = [...memoryAttendanceStore];
    if (AttendanceModel && AttendanceModel.findAll) {
      try {
        const dbList = await AttendanceModel.findAll();
        if (dbList && dbList.length > 0) {
          allRecords = dbList.map((item) => (item.toJSON ? item.toJSON() : item));
        }
      } catch (e) {}
    }
    const dayRecords = allRecords.filter((r) => r.date === today);
    ...
    if (!totalStaff || totalStaff === 0) {
      totalStaff = Math.max(8, dayRecords.length);
    }
